...
exports.up = function(knex, Promise) {
  return knex.schema.createTable('allpass_filters', function(table) {
    table.increments().primary();
    table.integer('user_id').notNullable().defaultTo(1).references('id').inTable('users').onDelete('CASCADE').index();
    table.string('name').notNullable().defaultTo('allpass filter');
    ...
    table.float('q', 4, 4).notNullable().defaultTo(1.0000);
...
exports.down = function(knex, Promise) {
  return knex.schema.dropTable('allpass_filters');
};
